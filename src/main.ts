...
async function main() {
  core.debug(
    `Triggered run for context=${JSON.stringify(github.context, null, 2)}`
  );
  ...
  const buildArgs = rawBuildArgs
    ...
    .filter((arg) => arg !== "");
  core.debug(
    `got rawBuildArgs='${rawBuildArgs}' translated into buildArgs=${buildArgs}`
  );
  ...
  const builds: Record<
    string,
    { b: Builder; tasks: Task[]; imageTags: string[] }
  > = {};
  ...
  if (numFailed > 0) {
    throw new Error(
      ...
      } builds failed. Review the table and logs above for more information.`
    );
  }

  console.log("Done. Ready to launch from https://app.airplane.dev 🛫");
  console.log(
    `These tasks can be run with your latest code using any of the following image tags: [${tags}]`
  );
}
...
async function getTags() {
  ...
  const defaultBranches =
    defaultBranch == null ? ["main", "master"] : [defaultBranch];
  core.debug(
    `Publishing :latest if defaultBranch=${defaultBranch} (-> ${defaultBranches}) is branch=${branch}`
  );
...
async function buildTask(
  ...
  core.debug(
    `wrote Dockerfile to ${dockerfilePath} with contents: \n${dockerfile}`
  );
